%analyze how many of the pooling feature map changed
    ...
    if(i > 1)
     ...
     %numPoolLayers = 1; %hard coded for testing !!!!!!!!!
      ...
      for j = 1 : numPoolLayers
	      ...
	      [unchangedRatio, unchangedFeatureMapRatio, maxUnchangedRatioPerMap] = comparePoolIdxMap(prevPMIdX, prevPMIdY, currentPMIdX, currentPMIdY);	      
		      

              fprintf(fid, 'unchanged ratio is %.4f\n', unchangedRatio);
              fprintf(fid, 'unchanged feature map ratio is %.4f\n', unchangedFeatureMapRatio);                                                                 
               fprintf(fid, 'max unchanged ratio per map is %.4f\n', maxUnchangedRatioPerMap);         
      end
...
%poolMapX = snapshotPoolIdxMapX('/data/vision/torralba/datasetbias/caffe-latest/examples/imagenet/caffe_object_train_iter_10000');                                                                                                            
%pm1 = poolMap('pool1');                                                                
