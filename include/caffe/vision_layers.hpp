...
template <typename Dtype>
class ConvolutionPoolingLayer : public Layer<Dtype> {
 public:
  /**
   * @param param provides ConvolutionParameter convolution_param,
   *    with ConvolutionLayer options:
   ...
   *    kernels + stream parallelism) engines.
   */
  explicit ConvolutionPoolingLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      vector<Blob<Dtype>*>* top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
 ...
 protected:
  ...
  int p_kernel_h_, p_kernel_w_;
  int p_stride_h_, p_stride_w_;
  int p_pad_h_, p_pad_w_;
  int p_channels_;
  int p_height_, p_width_;
  int pooled_height_, pooled_width_;
  Blob<Dtype> p_rand_idx_;
  Blob<int> p_max_idx_;
